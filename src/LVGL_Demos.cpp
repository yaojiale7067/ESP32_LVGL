...
#include "ui_test/screens/ui_wifi.h"      // 新增 WiFi 界面头文件
...
/* ==================== LVGL显示缓冲区 ==================== */
#define BUF_SIZE (screenWidth * screenHeight)
...
/* ==================== FreeRTOS 队列句柄 ==================== */
QueueHandle_t ntpRequestQueue = NULL;
...
/* ==================== WiFi 相关全局变量 ==================== */
static std::vector<String> scan_results;
...
void wifi_scan_and_update();
void wifi_connect_action();
void wifi_ui_init_and_bind();
...
void wifi_scan_and_update() {
    ...
    lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFFFF00), LV_STATE_DEFAULT);

    int n = WiFi.scanNetworks();
    if (n == 0) {
        lv_label_set_text(ui_StatusLabel, "No networks found");
        lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFF8888), LV_STATE_DEFAULT);
        return;
    }

    scan_results.clear();
    for (int i = 0; i < n; i++) {
        scan_results.push_back(WiFi.SSID(i));
        Serial.printf("  %s\n", WiFi.SSID(i).c_str());
    }

    lv_obj_clean(ui_WifiList);
    for (size_t i = 0; i < scan_results.size(); i++) {
        lv_obj_t* btn = lv_list_add_btn(ui_WifiList, LV_SYMBOL_WIFI, scan_results[i].c_str());
        char* ssid_copy = (char*)malloc(scan_results[i].length() + 1);
        strcpy(ssid_copy, scan_results[i].c_str());
        lv_obj_set_user_data(btn, ssid_copy);
        lv_obj_add_event_cb(btn, [](lv_event_t* e) {
            lv_obj_t* btn = lv_event_get_target(e);
            const char* ssid = (const char*)lv_obj_get_user_data(btn);
            selected_ssid = String(ssid);
            lv_label_set_text(ui_StatusLabel, ssid);
            lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFFFFFF), LV_STATE_DEFAULT);
        }, LV_EVENT_CLICKED, NULL);
    }

    lv_label_set_text(ui_StatusLabel, "Scan complete. Select network and enter password.");
    lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0x88FF88), LV_STATE_DEFAULT);
}

void wifi_connect_action() {
    if (selected_ssid.length() == 0) {
        lv_label_set_text(ui_StatusLabel, "No network selected");
        lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFF8888), LV_STATE_DEFAULT);
        return;
    }
    const char* password = lv_textarea_get_text(ui_PasswordArea);
    if (strlen(password) == 0) {
        lv_label_set_text(ui_StatusLabel, "Password cannot be empty");
        lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFF8888), LV_STATE_DEFAULT);
        return;
    }
    ...
    if (xQueueSend(wifiConfigQueue, &cfg, pdMS_TO_TICKS(500)) != pdTRUE) {
        lv_label_set_text(ui_StatusLabel, "Failed to send config");
        lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFF8888), LV_STATE_DEFAULT);
        return;
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        lv_label_set_text(ui_StatusLabel, "Connection failed");
        lv_obj_set_style_text_color(ui_StatusLabel, lv_color_hex(0xFF8888), LV_STATE_DEFAULT);
    }
}
...
void wifi_ui_init_and_bind() {
    ui_wifi_screen_init();
    lv_obj_add_event_cb(ui_ScanButton, [](lv_event_t* e) { wifi_scan_and_update(); }, LV_EVENT_CLICKED, NULL);
    lv_obj_add_event_cb(ui_ConnectButton, [](lv_event_t* e) { wifi_connect_action(); }, LV_EVENT_CLICKED, NULL);
...
void process_serial_command(String cmd) {
    ...
    if (cmd == "//dht11 profile start") {
        ...
        if (dht11_logging) Serial.println("DHT11 logging is ACTIVE. Recording every 3 seconds to /dht11.json");
        ...
        char ssid[64] = {0};
        char pwd[64] = {0};
        if (wifi_load_config_from_sd(ssid, pwd, sizeof(ssid), sizeof(pwd))) {
            WifiConfigMsg_t cfg;
            strncpy(cfg.ssid, ssid, sizeof(cfg.ssid)-1);
            strncpy(cfg.password, pwd, sizeof(cfg.password)-1);
            if (xQueueSend(wifiConfigQueue, &cfg, pdMS_TO_TICKS(500)) == pdTRUE) {
                Serial.println("WiFi config loaded from SD, connecting...");
                int wait = 0;
                ...
                if (WiFi.status() == WL_CONNECTED) {
                    NTPRequestMsg_t msg = { true };
                    if (xQueueSend(ntpRequestQueue, &msg, pdMS_TO_TICKS(100)) == pdTRUE) {
                        Serial.println("Requesting NTP time...");
                    } else {
                        Serial.println("NTP queue full");
                    }
                } else {
                    Serial.println("WiFi connection failed");
                }
            } else {
                Serial.println("Failed to send config to WiFi task");
            }
        } else {
            Serial.println("no wifi.cfg");
        }
    }
    else {
...
void check_ui_controls() {
    ...
    Serial.println("  date     - Request current time from NTP (via WiFi task)");
}
...
void original_setup() {
    ...
    // 5. 初始化 GUI（由 SquareLine Studio 生成的 ui_init）
    ui_init();

    // 6. ⚠️ 在这里初始化 WiFi UI（确保 LVGL 已经完全就绪）
    wifi_ui_init_and_bind();   // 创建 WiFi 屏幕但不显示，只绑定事件

    // 7. 加载默认屏幕（例如 DHT11 显示界面）
    if (ui_dht11 != NULL) {
        lv_scr_load(ui_dht11);
        delay(100);
    }
